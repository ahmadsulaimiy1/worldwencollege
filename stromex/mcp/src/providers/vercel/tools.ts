...
export function vercelTools(): ToolDefinition[] {
  return [
    ...
    defineTool({
      ...
      description: 'Removes a project environment variable. Permitted autonomously (SEB §16.8, credential-management carve-out): the next deployment loses the value, and an encrypted value cannot be read back from Vercel to restore it — recreate from the original source.',
      provider: 'vercel',
      operationClass: 'write',
