...
export function cloudflareTools(): ToolDefinition[] {
  return [
    ...
    defineTool({
      ...
      description: 'Removes a Worker secret. Permitted autonomously (SEB §16.8, credential-management carve-out): the Worker will fail at the first read of that binding, and the value cannot be recovered from Cloudflare — recreate with cloudflare.worker.secret.put from the original source.',
      provider: 'cloudflare',
      operationClass: 'write',
