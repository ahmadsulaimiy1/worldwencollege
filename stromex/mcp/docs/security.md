...
| Runaway automation exhausts a budget | Spending disabled by default; per-purchase and monthly caps | |
...
## 7. The HTTP transport

Off by default. When enabled it binds to loopback, requires a bearer
token in `STROMEX_MCP_HTTP_TOKEN`, and validates `Origin`. Do not expose
it to a network without putting an authenticating proxy in front of it,
and do not run it on a shared host at all.
