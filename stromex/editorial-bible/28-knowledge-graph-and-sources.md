...
│ Schools        │ │ College         │ │ International    │ │ College / WEC       │
...
| **Q7** | **Which domains are actually owned and renewed?** — **PARTLY ANSWERED 2026-08-18 by DNS, and narrowed by the Founder:** the estate's TLDs are `.com`, `.org` and `.co.uk`; `.ng`/`.com.ng` are **not** required. `worldwencollege.co.uk` is registered and live, entirely on Cloudflare (`mcp/docs/cost-model.md §7`). **Neither Vercel's nor Cloudflare's registrar API can handle `.co.uk`** (`SEB-D 36`), so the estate's primary domain can never be bought, renewed or transferred by this server. Still open: who holds the registration, when it renews, whether the registrar account is under estate control, and what other domains exist. | `SHRS shrs-digital-infrastructure-blueprint §1` says the primary domain is a hardcoded string, not evidence of registration. `WEC .env.example` records that worldwencollege.co.uk is the College's real, permanent domain and that sending mail from any other, unowned domain fails SPF/DKIM outright. | Volume 10's domain architecture; every email-sending workflow |
...
| **Q10** | ~~**Where should secrets actually live — 1Password, Vault, `gcloud`/`aws` secret managers, or an operator-owned file?**~~ **ANSWERED 2026-08-18 — `SEB-D 34`.** `pass`, the GPG-backed store, through the command-resolver seam. Rotation rehearsed before production. Runbook at `mcp/docs/credentials.md`. | The MCP supports all of them through one command-resolver seam and defaults to none. | `SEB §9.2`; installation |
