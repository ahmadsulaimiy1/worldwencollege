...
| **Q3** | **What is the spending authority — per project, per month, and which providers?** | A budget ceiling is on the escalation list at `SEB §0.5` by definition. Nothing in the corpus names a figure. | `SEB §26.6`; the MCP ships with automatic purchasing **disabled** until you set it |
...
| **Q9** | **Do you want the MCP to hold write credentials for every repository, or only for the ones under active work?** | A least-privilege question only you can settle, and it materially changes the blast radius of a compromised token. | `SEB §9.4`; the credential scoping in `stromex/mcp/docs/installation.md` |
