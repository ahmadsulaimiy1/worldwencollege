# WEC Master Roadmap — Executive Blueprint
...
- **🔭 Aspirational** — the long-term ambition from the founding brief. Real,
  ...
  counsel) that belong to WEC's operators, not to a coding session.
...
**What's actually true right now:** WEC has a complete, bilingual,
...
partners" the way `editorial-bible.md` already commits WEC not to do.
...
- **Risks:** proceeding to Phase 8+ without resolving #1 and #3 exposes
  WEC to real legal and financial risk the moment a real application is
  accepted.
...
- **MVP scope (Stage C):** account creation post-payment, a dashboard
  ...
  `mailto:`-based flows and lets WEC accept its first real cohort.
...
that recommendation — WEC now builds and owns its LMS as a
...
- **Original reasoning (overridden):** a bespoke LMS competing with
  ...
  investment for capability WEC's brand doesn't actually depend on —
  ...
  asset rather than a rented commodity, and WEC's own engineering
...
Faculty/Administration surfaces of WEC's own LMS (gradebook,
...
- **Recommendation (partially superseded by the above):** this phase's
  ...
  already cover" (WEC-specific reporting,
...
- **Recommended, bounded uses:** placement-test scoring assistance
  ...
  site/portal support chatbot answering from WEC's actual published
  ...
  doubly to anything AI-generated and published under WEC's name).
...
- **Company name risk.** "University" cannot be used in a UK company name
  ...
  WEC as a UK entity under this name.
...
- **Immigration/visa sponsorship — good news, not a gap.** A UK Student
  ...
  UK. As currently marketed ("100% Online Digital Campus"), WEC likely
...
WEC currently has no named leadership at all — not even a founder
...
- **Why this matters even as a placeholder:** it suggests WEC's model
...
- **Cash-flow risk, and a concrete fix.** Phase 8/9/11 (the MVP portal +
  ...
  **Stripe Payment Link** (zero engineering, live in under an hour) and
  ...
  This "manual bridge" lets WEC accept paying students *now*, prove
...
- **The "London Campus" vs. "100% Online" tension.** The site currently
  states, in the same breath, that WEC is a "**100% Online Digital
  Campus**" (home page hero stat) and is named "**Worldwide English
  College — *London Campus***," with a physical
...
1. **Run the Companies House sensitive-words + UK IPO trademark search**
   for "Worldwide English College," "WEC," and "IEFC" — free-to-cheap,
...
from the outset given WEC's global, Nigeria-inclusive target market.
...
question (former #6) is resolved as build, not buy** — WEC develops
...
2. **A real GBP exchange rate or policy-fixed rate** — the currency
   *strategy* (multi-currency, GBP/USD primary) is decided; GBP itself
   ...
   or WEC sets a policy-fixed rate — see `payments-architecture.md`
...
6. ~~**LMS: buy or build?**~~ — resolved: build. WEC's own proprietary
