# AIPC — English Curriculum Framework
...
framework). This is AIPC's own curriculum design — informed by the
...
accreditation (AIPC holds none — see `docs/editorial-bible.md`'s
...
Resolved. You confirmed AIPC retains its existing, published
...
AIPC's programme is not a grammar-topic checklist. Four principles
...
— the CEFR A1 "breakthrough" descriptor, applied to AIPC's own
scope and sequence.
...
the CEFR A2 "waystage" descriptor, applied to AIPC's scope.
...
"effective operational proficiency" descriptor, applied to AIPC's
scope.
...
**Distinctive AIPC identity:** per your directive, Level V does not
...
style/voice mastery → consolidation) is a deliberately AIPC-authored
...
VI directive, this level is *the capstone of the AIPC academic
...
eliminating accent — AIPC does not treat "sounding native" as the
...
CEFR C2 "mastery" descriptor, applied to AIPC's scope, and the
programme's terminal, publishable outcome: a AIPC IEFC certificate
...
duration) or AIPC's own original curriculum design, written for this
