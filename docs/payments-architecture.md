...
2. AIPC sets a **policy-fixed** rate (common for tuition — many
...
- **`fx-service.js`** — `refreshFromLiveFeed()` (calls a provider,
  ...
  Nigeria currencies, and an alternative for GBP too if AIPC prefers
...
`users.role IN ('staff','admin')`, a AIPC-owned field, not anything
...
**What this deliberately does not do:** compare AIPC's own records
