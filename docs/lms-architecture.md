# WEC — Learning Management System Architecture

*Companion to `technical-architecture.md` and `payments-architecture.md`.
Authoritative record of Executive Decision #4: WEC develops and
...
Not a rebuilt Canvas/Moodle. The scope is exactly what WEC's own
...
uses it — see § Deliberate MVP scope below — but the platform WEC
...
(`functions/_lib/auth/session.js`) — role lives on WEC's own
...
real WEC curriculum, and lives only in the test file — nothing
...
establishing WEC as a premium international English institution,
...
WEC-authored module sequence (precision of language → persuasion →
...
your Level VI directive, which asked for the capstone of the WEC
...
criterion that distinguishes C2 from C1 in WEC's assessment model —
...
wouldn't produce better outcomes for WEC students and would be a
...
- **What genuinely is WEC's own IP, and is built accordingly:** the
