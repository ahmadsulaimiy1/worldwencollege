...
const CHARACTER = {
  ...
  II: 'Elementary is where isolated sentences become connected speech. The learner already has the present and the past; this level teaches them to join, sequence and qualify. That is telling a story rather than listing events, giving an opinion rather than a fact, and keeping a conversation going when the other person says something unexpected.',
...
${card('Graduate profile', 'What the holder can do', esc(a.graduate_profile), 'i-mortarboard')}
    ...
    <div class="callout">
      ...
      <p>Every award at this level is set, marked and second-marked inside the College, against
        the criteria published above and issued before the work is attempted. Moderation is
        held by the Assessment Integrity Committee under the College&rsquo;s own academic
        authority, and the Independent External Examiner&rsquo;s office carries the standing
        remit to review assessment frameworks, examination standards and award classifications
        from outside it. See <a href="/governance/quality/">The Quality Architecture</a>.</p>
    </div>
  </div>
</section>` : '';
...
<section class="section--dark section-pad" id="progression">
  <div class="container reveal">
    ...
    <!-- THE ASCENT, with this level struck. Deliberately not a progress
         bar: a reader arriving here has not completed the rungs below,
         so every rung is drawn at the same weight and only the one this
         page is about is marked. Roman numeral, CEFR band and award
         post-nominal are read from programme_levels and
         award_definitions by scripts/art/generate-level-ascent.mjs, so a
         level renamed in the record is renamed in all six drawings. -->
    <figure class="diagram diagram--wide">
      {{SVG:assets/art/level-ascent-${lv.id}.svg}}
      <figcaption class="diagram__caption">
        <svg class="icon" aria-hidden="true"><use href="#i-compass"/></svg>
        Six rungs, and every one of them a finished award
      </figcaption>
    </figure>
