...
/* THE RUBRIC IS READ, SO IT IS SET IN INK AND NOT IN ACCENT.
   The accent token on the pale palettes is a champagne gold: correct
   for a mark, a rule or a numeral, and 1.55:1 on cream at 13.4 point.
   A part opener's one sentence of argument was the faintest text in
   the book, and the build now fails on it. */
.op2__rub { font-family: ${FACE.display}; font-style: italic; font-weight: 400; font-size: 13.4pt;
  line-height: 20pt; color: var(--title); margin: 9mm 0 0; padding-top: 6mm;
  border-top: .5pt solid var(--rule); max-width: ${M.col(9)}mm; }
/* THE DRAWING YIELDS TO THE CONTENTS, NOT THE OTHER WAY ROUND.
   Once the rail joined the flow, three openers overflowed — which is
   what the absolute positioning had been hiding rather than
   preventing. The datum is the one element on the page with no fixed
   size of its own, so it is the one that gives: it takes the height
   left after the title, the rubric, the caption and the rail, and the
   drawing scales down inside it rather than pushing anything off the
   leaf. */
.op2__datum { margin: 16mm 0 0; min-height: 0; flex: 0 1 auto;
...
/* IN THE FLOW, NOT FLOATING OVER IT. The contents rail was absolutely
   positioned at the foot, so it climbed as it grew and printed
   straight through the caption above it — which is what happened the
   moment Part VI gained three leaves. Pinned with an auto top margin
   instead, it pushes rather than overlaps, and a rail that no longer
   fits overflows the field, which the build already fails on. */
.m-open2 .pg__field { display: flex; flex-direction: column; }
.op2__rail { margin-top: auto;
