...
const AR_CHARACTER = {
  ...
  II: 'في المستوى الابتدائي تتحول الجمل المنفصلة إلى كلام متصل. المتعلم يملك المضارع والماضي، '
    + 'وهذا المستوى يعلّمه الربط والترتيب والتقييد. أي أن يروي بدل أن يعدّد، وأن يبدي رأيًا لا '
    + 'أن يذكر واقعة، وأن يواصل الحديث حين يقول محدّثه شيئًا غير متوقع.',
...
<section class="section--dark section-pad" id="progression">
  <div class="container reveal">
    ...
    <!-- السُّلَّم، وهذا المستوى محدَّد عليه. ليس شريط تقدُّم عن قصد: من
         يصل إلى هذه الصفحة لم يُتمّ الدرجات دونه، فكل الدرجات مرسومة
         بالثقل نفسه ولا يُعلَّم إلا ما تتناوله الصفحة. الأرقام والأسماء
         مقروءة من السجل بـ scripts/art/generate-level-ascent.mjs. -->
    <figure class="diagram diagram--wide">
      {{SVG:assets/art/level-ascent-${lv.id}.ar.svg}}
      <figcaption class="diagram__caption">
        <svg class="icon" aria-hidden="true"><use href="#i-compass"/></svg>
        ست درجات، كل واحدة منها شهادة تامة
      </figcaption>
    </figure>
...
${card('الالتزام', 'أستطيع الدراسة مع عملي؟', `نعم. على هذه الهيئة بُني. المستوى ${ord} ${ltr(String(TQT_HOURS))} ساعة مصمَّمة عبر ${ltr(String(lv.duration_months))} أشهر — نحو ${ltr(String(weeklyHours(lv)))} ساعات أسبوعيًا، تصرفها حين تختار أن تصرفها. ولا شيء يُفتح في تاريخ محدد ولا شيء يُغلق إن ساء أسبوع.`, 'i-hourglass')}
    </div>
  </div>
</section>
