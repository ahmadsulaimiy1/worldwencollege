...
const nodes = LEVELS.map((lv, i) => {
  ...
  return `    <g data-pop>
      ...
      <text x="${fmt(x + dx)}" y="${fmt(y + 15)}" text-anchor="${anchor}"
            font-family="${SANS}" font-size="12.5" fill="#8FA3C4">${lv.name}</text>
    </g>`;
}).join('\n');
...
const spokeLabels = spokes.map((s) => {
  ...
  return `    <text data-pop x="${fmt(s.lx)}" y="${fmt(s.ly)}" text-anchor="${anchor}"
          ...
          fill="#6E93C4" opacity=".85">${RTL ? s.label : s.label.toUpperCase()}</text>`;
}).join('\n');

const svg = `<?xml version="1.0" encoding="UTF-8"?>
<!-- Generated by scripts/art/generate-curriculum-spiral.mjs — do not hand-edit. -->
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${W} ${H}" width="${W}" height="${H}"
     data-diagram="curriculum-spiral" role="img" lang="${LANG}"
     aria-labelledby="spiral-title spiral-desc">
  <title id="spiral-title">${TITLE[LANG] || TITLE.en}</title>
  <desc id="spiral-desc">${DESC[LANG] || DESC.en}</desc>
...
${turns}
  ...
  <g data-pop>
    ...
    <text x="${CX}" y="${CY - 2}" text-anchor="middle"
          ...
          fill="#C7A24A">${(CENTRE[LANG] || CENTRE.en)[0]}</text>
    <text x="${CX}" y="${CY + 11}" text-anchor="middle"
          font-family="${SANS}" font-size="${RTL ? 11 : 9.5}" font-weight="700" letter-spacing="${RTL ? 0 : 1.8}"
          fill="#C7A24A">${(CENTRE[LANG] || CENTRE.en)[1]}</text>
  </g>
