...
const curriculumSeed = readFileSync(`${ROOT}/sql/seed-curriculum-level-2.sql`, 'utf8');
...
for (const unit of units) {
  ...
  const graded = await gradeAssignment(env, { gradedBy: 'usr_staff', submissionId: submission.id, grade: 0.85, feedback: 'Strong communicative competence.' });
...
// --- A weak attempt correctly fails, not a false pass (Module 1's quiz) ---
{
  ...
  db.prepare(`INSERT INTO enrolments (id, user_id, level_id, status, started_at) VALUES ('enr_weak_l2', 'usr_weak', 2, 'active', '2026-01-01T00:00:00.000Z')`).run();
  const wrongAnswers = Array(10).fill(3);
  const attempt = await submitQuizAttempt(env, { userId: 'usr_weak', learningItemId: 'itm_l2_m1_quiz', answers: wrongAnswers });
