 ...
 * number.
 *
 * Pricing the projection by region cost the plan $22.4M of ten-year
 * revenue and $17.2M of surplus, and moved the year the institution
 * turns. That is not a loss; it is the removal of revenue the College
...
/* Placement moves seats and prices them; it must not conjure or lose
   any. */
...
check('placing agreements by region conserves the number of seats',
  Math.abs(chGlobal - chRegional) <= 2, `${chGlobal} against ${chRegional}`);
