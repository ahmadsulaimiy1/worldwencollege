// Run with: node --experimental-sqlite tests/curriculum-level-4.test.mjs
// Covers Level IV (Upper Intermediate Programme, B2) — currently
// Modules 1-9 (see docs/curriculum-level-4-upper-intermediate.md's
// module map; Module 10 is mapped but not yet authored/seeded).
// Same pattern as tests/curriculum-level-3.test.mjs: sweep every
// seeded module, submit each quiz's own real seeded correct answers
// (read directly from the DB, not hand-copied), confirm a perfect
// score, no leaked answer key, and that assignments can be submitted
// and graded.
...
const curriculumSeed = readFileSync(`${ROOT}/sql/seed-curriculum-level-4.sql`, 'utf8');
...
db.prepare(`INSERT INTO enrolments (id, user_id, level_id, status, started_at) VALUES ('enr_student_l4', 'usr_student', 4, 'active', '2026-01-01T00:00:00.000Z')`).run();

const units = await listUnits(env, { userId: 'usr_student', levelId: 4 });
check('Level IV has the 9 modules built so far (Modules 1-9)', units.length === 9);
check('Modules are in the correct sequence order', units.every((u, i) => u.sequence === i + 1));
const expectedTitles = {
  1: 'Module 1: Advanced Present & Past Systems',
  2: 'Module 2: Academic Writing I',
  3: 'Module 3: The World of Work',
  4: 'Module 4: Arguing a Position',
  5: 'Module 5: Science, Technology & Ethics',
  6: 'Module 6: Global Issues',
  7: 'Module 7: Media Literacy & Critical Reading',
  8: 'Module 8: Meetings & Negotiation',
  9: 'Module 9: Academic Writing II',
};
...
for (const unit of units) {
  ...
  const graded = await gradeAssignment(env, { gradedBy: 'usr_staff', submissionId: submission.id, grade: 0.85, feedback: 'Strong evidence and argument quality, appropriate register.' });
  check(`Module ${moduleNum}: assignment can be graded by staff`, graded.status === 'graded' && graded.grade === 0.85);
}

check('Every quiz question across all built Level IV modules was verified against its real seeded answer key', totalQuestionsChecked === 90);
...
// --- A weak attempt correctly fails, not a false pass (Module 1's quiz) ---
{
  ...
  db.prepare(`INSERT INTO enrolments (id, user_id, level_id, status, started_at) VALUES ('enr_weak_l4', 'usr_weak', 4, 'active', '2026-01-01T00:00:00.000Z')`).run();
  const wrongAnswers = Array(10).fill(3);
  const attempt = await submitQuizAttempt(env, { userId: 'usr_weak', learningItemId: 'itm_l4_m1_quiz', answers: wrongAnswers });
