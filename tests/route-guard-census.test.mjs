...
const PUBLIC = {
  ...
  'health/auth.js': 'The deployment-readiness probe, and it has to be unauthenticated because the failure it diagnoses IS the failure to authenticate — an endpoint you can only reach once sign-in works cannot tell you why sign-in does not work. It was added after an applicant signed in successfully and was told their application "could not be loaded, this is usually temporary" for a deployment with no CLERK_JWKS_URL, where retrying could never have worked. It returns booleans and fixed sentences only: no secret, no environment value, no personal data, and no more about the infrastructure than the publishable key already shipping in every page reveals. tests/admissions-availability.test.mjs asserts the no-secret property directly, including that the JWKS path is not echoed.',
