// FIFTEEN NAMED PEOPLE, AND NOTHING ELSE.
//
...
//     said no award can be conferred still says it. Filling fifteen
...
check(`The register holds fifteen people — ${ALL.length}`, ALL.length === 15,
...
//
// The scan reads fourteen of the fifteen. "Ahmad Sulaimi" carries no
...
// residual hole is that a sixteenth person written WITHOUT a title could
...
// Prove the scan reaches. A sixteenth person spliced into the page must
// be caught by the same comparison, not just by a count.
{
  ...
  const found = [...new Set(sabotaged.match(PERSON) || [])].filter((n) =>
    !known.has(n) && ![...known].some((k) => k.startsWith(n) || n.startsWith(k)));
  check('...and that comparison does catch a sixteenth name spliced into the page',
...
// Five of the six executive officers were attested without
...

{
  ...
  check(`The register really does carry unsupplied credentials — ${blanks.length} of them`,
    blanks.length >= 5, blanks.map((r) => r[0]).join(', '));
}
...
// treatment tests/bilingual-links.test.mjs gives the Arabic backlog.
{
  ...
  const clashes = ALL
    .filter((r) => facultyPosts.has(r[1]) && facultyPosts.get(r[1]) !== r[0])
  ...
  check('Every post held by two different people is recorded in the register\'s collisions table',
    clashes.every((c) => registerDoc.includes(c.split(':')[0])),
    clashes.filter((c) => !registerDoc.includes(c.split(':')[0])).join(' · '));

  if (clashes.length) {
    console.log(`\nNOTE ${clashes.length} post(s) are held by a different person in each register:`);
    for (const c of clashes) console.log(`     ${c}`);
    console.log('     Recorded in docs/governance-register.md under "Collisions". Only the');
    console.log('     College can settle these — not a failure, a question.');
  }
}
