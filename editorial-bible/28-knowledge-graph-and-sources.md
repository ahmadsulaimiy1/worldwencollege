...
| **Q3** | ~~**What is the spending authority — per project, per month, and which providers?**~~ **ANSWERED 2026-08-18 — `SEB-D 28`.** USD; US$25 single maximum; providers named; the monthly cap remains open pending a costing. Not in effect until `SEB-D 29` is discharged. | A budget ceiling is on the escalation list at `SEB §0.5` by definition. Nothing in the corpus names a figure. | `SEB §26.6`; the MCP ships with automatic purchasing **disabled** until you set it |
