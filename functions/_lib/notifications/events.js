...
const TEMPLATES = {
  application_received: (rawData) => {
    ...
    return {
      subject: 'We\'ve received your WEC application',
  ...
  payment_failed: (rawData) => {
    const data = esc(rawData);
    return {
      subject: 'Payment unsuccessful — WEC',
  ...
  enrolment_confirmed: (rawData) => {
    const data = esc(rawData);
    return {
      subject: 'You\'re enrolled — welcome to WEC',
  ...
  new_application_alert: (rawData) => {
    ...
    return {
      ...
      subject: `New WEC application — ${sanitizeHeaderText(rawData.name)}`,
...
// For events WEC staff need to see (a new application, say) rather
