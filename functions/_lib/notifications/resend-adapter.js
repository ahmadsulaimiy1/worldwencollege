// Resend adapter — the FALLBACK provider, not the live one. Brevo is
// the provider the College can actually send from; see the note at the
// head of brevo-adapter.js for why, and events.js for the runtime
// switch (Brevo when BREVO_API_KEY is set, this otherwise).
//
// Kept wired rather than deleted: the constraint that rules Resend out
// is a free-tier domain limit on one particular account, not anything
// about the API, so this becomes usable again the moment that changes.
// Requires RESEND_API_KEY and RESEND_FROM_ADDRESS (e.g.
